classdef NRRD < crlEEG.file.basefile & matlab.mixin.Copyable
  ...
  methods
    ...
    function obj = NRRD(fname,fpath,readonly)
      ...
      if (needsFName && needsFPath)
        % If name and path are not provided, use a temporary file name in
        % the /tmp/ directory
        fname = [name '.nrrd'];
        fpath = path;
      elseif needsFName
        % Default filename is a random temporary name
        fname = [name '.nrrd'];
      elseif needsFPath
        % Default path when a filename is provided is the current directory
        fpath = './';
      end;
