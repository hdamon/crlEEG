classdef cnlMatrixOnSpace < cnlAdaptableMatrix
  
  % function obj = cnlMatrixOnSpace(matrix,solSpace,colPerVox)
  %
  % Inputs:
  %   matrix      :
  ...
  %
  % Properties:
  %   Inherited:  origMatrix     < cnlAdaptableMatrix
  %               currMatrix     < cnlAdaptableMatrix
  %               isTransposed   < cnlAdaptableMatrix
  %               needsRebuild   < cnlAdaptableMatrix
  ...
  methods
    function obj = cnlMatrixOnSpace(matrix,solSpace,colPerVox)
      % function obj = cnlMatrixOnSpace(matrix,solSpace,colPerVox)
      %
      % Constructor function for cnlMatrixOnSpace class.
      %
      % Inputs:
      ...
      obj = obj@cnlAdaptableMatrix;
      
      if nargin>0
        if ~(isnumeric(matrix)&&exist('solSpace','var')&&isa(solSpace,'cnlSolutionSpace'))
          error(['cnlMatrixOnSpace can be called in exactly one way, to ' ...
    ...
    function obj = set.stillLoading(obj,val)
      ...
      warning('cnlMatrixOnSpace.stillLoading is deprecated. Use obj.disableRebuild instead');
      obj.disableRebuild = val;
    end;
  ...
  methods (Static = true)
    ...
    function obj = loadobj(S)
      obj = cnlMatrixOnSpace;      % Initialize Empty Object
