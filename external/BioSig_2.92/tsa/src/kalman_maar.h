...
struct Kalman_Helper
{
	Kalman_Helper( mwSignedIndex M, mwSignedIndex p )
	{
		mwSignedIndex L = M*M*p;
		ye = new double[M];
		ipiv = new mwSignedIndex[M];
		tmp = new double[M*M];
		KpH = new double[L*M];
		HKp = new double[M*L];
		HKpH = new double[M*M];
...
void kalman_maar( double *err, const double *y,
	...
	double UC, mwSignedIndex M, mwSignedIndex L, mwSignedIndex p,
	struct Kalman *inKalman, struct Kalman *outKalman,
	struct Kalman_Helper *kh, int mode )
{
	double traceK;
	double *tmp1, *tmp2, *tmp3;
	mwSignedIndex n,i,j, info, index;
